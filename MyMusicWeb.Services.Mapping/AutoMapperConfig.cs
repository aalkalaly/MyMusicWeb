...
namespace MyMusicWeb.Services.Mapping
{
    ...
    public class AutoMapperConfig
    {
        ...
        private static IEnumerable<IHaveCustomeMapping> GetCustomMappings(IEnumerable<Type> types)
        {
            var customMaps = from t in types
                             from i in t.GetTypeInfo().GetInterfaces()
                             where typeof(IHaveCustomeMapping).GetTypeInfo().IsAssignableFrom(t) &&
                                   !t.GetTypeInfo().IsAbstract &&
                                   !t.GetTypeInfo().IsInterface
                             select (IHaveCustomeMapping)Activator.CreateInstance(t);
